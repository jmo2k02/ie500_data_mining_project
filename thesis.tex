...
\section{Data Origin}
To build a robust prediction model, we require a synthesis of historical flight logs and synchronized environmental data. The data gathering process is divided into two primary streams: Aviation Operational Data and Meteorological Data.
\subsection{Data Source}
We have identified three primary tiers of data providers, each offering a different balance of cost and granularity:
- BASED ON "On time Carrier and Flight Delay Data", from the American Buero of Transoiortaton Statsikis.x
    - Contains flight on Time Perfomance, for every ameruican Flight
    -  Features Describe Flight Data, and On time Performance for aspect of the flight
- Airport Datasets, with Basic in infomration about the Airport by OurAirports.com for feature engeneering.
    - e.g. Every Aiport in the World, with ther Loctaion Data, Size Type
- Weather Hourly Weather Information from Meteostat libray
    - hourly data or airport or nearby weather staions, on tempeeratur, windspead, precipcation, snow depth, and various other parameters
- other posible small other data sources to match to the data, for further features, 
\subsection{Data Gathering}
- we get the Base data as CSV files, and the Airports data aswell, while the Weather data is gathered via an liabry api
we select Our base Data over a 10 Year Period, and used the "IATA" codes to match the Airpot and secondly the Weather Data to the flights.
